...
/* ── TOC items ── */
const tocSections = [
  { id: "aug-summary", label: "KPI・サマリー", items: [
    { id: "aug-kpi", label: "KPI" },
    { id: "aug-cross", label: "今月やるべきこと" },
  ]},
  { id: "aug-measures", label: "施策", items: [
    { id: "aug-schedule", label: "週別スケジュール" },
    { id: "aug-events", label: "イベント" },
  ]},
  { id: "aug-procurement", label: "仕入予算配分", items: [
    { id: "aug-budget", label: "予算" },
    { id: "aug-products", label: "新作品番" },
    { id: "aug-restock", label: "再入荷" },
    { id: "aug-clearance", label: "OFF消化" },
  ]},
  { id: "aug-analysis", label: "分析", items: [
    { id: "aug-axis1", label: "軸①ZOZO" },
    { id: "aug-axis2", label: "軸②楽天" },
    { id: "aug-axis3", label: "軸③トレンド" },
    { id: "aug-axis4", label: "軸④自社実績" },
    { id: "aug-patterns", label: "成功/失敗" },
  ]},
  { id: "aug-competitors", label: "他社売れ筋ランキング", items: [] },
];
...
/* ══════════════════════════════════════════
   August Section — Action Dashboard + Market Data
   ══════════════════════════════════════════ */

export default function AugustSection({ plan }: { plan: MonthPlan }) {
      ...
      <div className="bg-base-card rounded-2xl border border-brand/10 shadow-[0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        <div className="relative bg-gradient-to-r from-brand/12 via-brand/5 to-transparent px-6 py-6 border-b border-brand/10">
          <div className="flex items-end gap-3 mb-1">
            <span className="text-[52px] font-extrabold text-brand leading-none tabular-nums">8</span>
        ...
        <nav className="px-6 py-4 border-b-2 border-brand/10 bg-gradient-to-b from-base-card to-base/80">
          ...
          <div className="flex items-center gap-2.5 overflow-x-auto scrollbar-none">
            {tocSections.map((sec) => {
              const sectionColor = sec.id === "aug-summary" ? "bg-brand/15 text-brand-dark border-brand/30 hover:bg-brand/25"
                : sec.id === "aug-measures" ? "bg-blue-50 text-blue-700 border-blue-300 hover:bg-blue-100"
                : sec.id === "aug-procurement" ? "bg-emerald-50 text-emerald-700 border-emerald-300 hover:bg-emerald-100"
                : sec.id === "aug-analysis" ? "bg-purple-50 text-purple-700 border-purple-300 hover:bg-purple-100"
        ...
        <div className="p-6 space-y-10">
          ...
          {/* ════════════════════════════════════════
             ◆ KPI・サマリー
             ════════════════════════════════════════ */}
          <div id="aug-summary" className="rounded-2xl bg-brand/15 border-l-4 border-brand px-6 py-5 mb-4 scroll-mt-24">
          ...
          {/* ── KPI ── */}
          <div id="aug-kpi">
            <SectionHeader id="aug-kpi-h">KPI</SectionHeader>
          ...
          {plan.crossAnalysis && plan.crossAnalysis.length > 0 && (
            <div id="aug-cross">
              <SectionHeader id="aug-cross-h">今月やるべき3つのこと</SectionHeader>
          ...
          {/* ════════════════════════════════════════
             ◆ 施策
             ════════════════════════════════════════ */}
          <div id="aug-measures" className="rounded-2xl bg-blue-100 border-l-4 border-blue-500 px-6 py-5 mb-4 scroll-mt-24">
          ...
          {plan.weeklyPlan && plan.weeklyPlan.length > 0 && (
            <div id="aug-schedule">
              <SectionHeader id="aug-schedule-h">週別スケジュール</SectionHeader>
          ...
          {/* ── Events (under 施策) ── */}
          <div id="aug-events" className="rounded-xl bg-base p-5">
            <SectionHeader id="aug-events-h">イベント・施策</SectionHeader>
          ...
          {/* ════════════════════════════════════════
             ◆ 仕入予算配分
             ════════════════════════════════════════ */}
          <div id="aug-procurement" className="rounded-2xl bg-emerald-100 border-l-4 border-emerald-500 px-6 py-5 mb-4 scroll-mt-24">
          ...
          {/* ── Budget Detail Table ── */}
          <div id="aug-budget">
            <SectionHeader id="aug-budget-h">予算検算{plan.totalBudget ? `（合計 ${plan.totalBudget}）` : ""}</SectionHeader>
            {plan.budgetDetail ? (() => {
          ...
          <div id="aug-products">
            <SectionHeader id="aug-products-h">新作品番設計（{plan.products.length}型）</SectionHeader>
            {(() => {
                      ...
                      <div key={g.label} className={`border-l-4 ${seasonColor} rounded-r-xl pl-4 py-3`}>
                        ...
                        <div className="space-y-3">
                          {products.map((p) => {
                            ...
                            return (
                              <details key={p.id} id={`aug-product-${p.id}`} className="rounded-xl border border-brand/10 overflow-hidden group scroll-mt-24 bg-base-card">
          ...
          {plan.restockItems && plan.restockItems.length > 0 && (
            <div id="aug-restock">
              <SectionHeader id="aug-restock-h">再入荷リスト（夏¥400万＋秋¥550万）</SectionHeader>
              <div className="overflow-x-auto">
                <table className="w-full text-[14px]">
                  <thead>
                    <tr className="bg-base">
          ...
          {plan.clearanceCandidates && plan.clearanceCandidates.length > 0 && (
            <div id="aug-clearance">
              <SectionHeader id="aug-clearance-h">OFF消化候補</SectionHeader>
              <div className="overflow-x-auto">
                <table className="w-full text-[14px]">
                  <thead>
          ...
          {/* ════════════════════════════════════════
             ◆ 分析
             ════════════════════════════════════════ */}
          <div id="aug-analysis" className="rounded-2xl bg-purple-100 border-l-4 border-purple-500 px-6 py-5 mb-4 scroll-mt-24">
          ...
          {plan.marketReports && (() => {
            ...
            const axisGroups = [
              { id: "aug-axis1", num: "①", title: "ZOZO市場分析", color: "bg-gray-800", reports: zozoReports },
              { id: "aug-axis2", num: "②", title: "楽天市場分析", color: "bg-red-500", reports: rakutenReports },
              { id: "aug-axis3", num: "③", title: "2026トレンド分析", color: "bg-purple-500", reports: trendReports },
              { id: "aug-axis4", num: "④", title: "自社実績分析", color: "bg-brand", reports: selfReports },
            ];
          ...
          {(plan.successPatterns || plan.failurePatterns) && (
            <div id="aug-patterns" className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {plan.successPatterns && (
                <div className="rounded-xl bg-base p-5 border-l-4 border-emerald-400">
                  <SectionHeader id="aug-success-h">成功パターン</SectionHeader>
                  <div className="space-y-3">
              ...
              {plan.failurePatterns && (
                <div className="rounded-xl bg-base p-5 border-l-4 border-red-400">
                  <SectionHeader id="aug-failure-h">失敗パターン</SectionHeader>
                  <div className="space-y-3">
          ...
          {plan.competitorProducts && plan.competitorProducts.length > 0 && (<>
            <div id="aug-competitors" className="rounded-2xl bg-gray-200 border-l-4 border-gray-500 px-6 py-5 mb-4 scroll-mt-24">
